...
import isDocumentFragment from "./utils/isDocumentFragment.utils.js";
import isStatixDOMManipulation from "./utils/isStatixDOMManipulation.utils.js";
...
class StatixDOM {
	 ...
	 *	@param   {string | HTMLElement | StatixDOMManipulation} tagOrStatixDOMManipulationOrElement
	 *	@returns {void}
	 */
	setRoot(tagOrStatixDOMManipulationOrElement) {
		if(isString(tagOrStatixDOMManipulationOrElement)) {
			this.#mRoot = document.querySelector(tagOrStatixDOMManipulationOrElement);
		} else if(isElement(tagOrStatixDOMManipulationOrElement) || isDocumentFragment(tagOrStatixDOMManipulationOrElement)) {
			this.#mRoot = tagOrStatixDOMManipulationOrElement;
		} else if(isStatixDOMManipulation(tagOrStatixDOMManipulationOrElement)) {
			this.#mRoot = tagOrStatixDOMManipulationOrElement.__mElement__;
		}
	}
	/**
	 * 
	...
	/**
	 *	Sets Fragment element for processing.
	 *	@returns {StatixDOMManipulation}
	 */
	fragment() {
		return new StatixDOMManipulation(document.createDocumentFragment(), this.#mStatix);
	}
	/**
   *	If a string is passed, only the `create` function can be called. `create` function returns the created element.\
	...
	element(tagOrElement) {
		return new StatixDOMManipulation(
			(isElement(tagOrElement) || isDocumentFragment(tagOrElement)) ? tagOrElement :
			isStatixDOMManipulation(tagOrElement) ? tagOrElement.__mElement__ : 
			document.createElement(tagOrElement), this.#mStatix);
	}
	/**
			...
			if((currArrayItem && prevArrayItem && childMustBeChangedCallback) && 
			   (child?.dataset[G_STATIX_DATASET_LIST_ID] === key) && 
				...
				if(newChild instanceof StatixDOMManipulation) {
					...
					element.appendChild(newChild.__mElement__);
				} else {
