  ...
  /**
	...
	getCache(key) {
		...
		if(isStatixDOMManipulation(data)) {
			return data.__mElement__.cloneNode(true);
	...
	signal(initValue) {
		...
		const emit = () => signal__emit__(signalData, this);

		return { val, set, subscribe, unsubscribe, emit };
	}
	/**
	 *	@param {string} key
	 *		@param {any} data 
	...
	setCache(key, data) {
		if(isStatixDOMManipulation(data)) {
			this.#mCache[key] = data.__mElement__.cloneNode(true);
		} else if(isElement(data)) {
