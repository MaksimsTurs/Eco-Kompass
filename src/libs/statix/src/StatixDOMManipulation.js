...
class StatixDOMManipulation {
	 ...
	 */
	style(styles) {
		for(let styleKey in styles[0]) {
			this.__mElement__.style[styleKey] = styles[0][styleKey];
		}
	 ...
	 */
	dataset(datasets) {
		for(let datasetsKey in datasets[0]) {
			this.__mElement__.dataset[datasetsKey] = datasets[0][datasetsKey]; 
		}
	...
	toggleClass(selector) {
		if(selector) {
			this.__mElement__.classList.toggle(selector);
		}
